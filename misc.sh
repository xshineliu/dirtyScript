...
for i in $(find /var/run/topinfo/010_100_100_000 -name "*.log"  | sort -n); do echo -ne $i" "; grep "qemu" $i > /dev/null; if [ $? -eq 0 ]; then echo -ne "QEMU "; else  echo -ne "---- "; fi; echo $(stat $i | grep Mod | awk '{print $2, $3}' | cut -b -19); done
